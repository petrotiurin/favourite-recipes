...
1. <One action per step, sentence case.>
```
...
- **Bold the quantity + ingredient name**, e.g. `**150g (⅔ cup) Greek yogurt**, or `**2 celery sticks**, finely diced`. Prep instructions/notes after the ingredient (diced, finely chopped, to taste, etc.) stay outside the bold.
...
- Resize so the long edge is ≤ 1600px before saving, to keep the repo and page weight small. If you're given a much larger source image, downscale it (e.g. with `sips` on macOS or `ffmpeg`/`magick` if available) rather than committing it as-is.
...
Confirm it completes without error, then open `dist/index.html` (or the new `dist/recipes/<slug>.html`) and check: the new tile appears with its image and time, the ingredient bullets render with bold quantities, and the instructions are numbered correctly.
